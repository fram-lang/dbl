...
let tr_uvar (x : UID.t) =
  Sym (Printf.sprintf "weak%s" (UID.to_string x))
...
and tr_arrow : ttype -> SExpr.t list =
  ...
  | TUVar _ | TUnit | TVar _ | TForall _ | TLabel _ | TData _ | TApp _ ->
    [ Sym "->"; tr_type tp ]
...
and tr_forall : ttype -> SExpr.t list =
  ...
  | TUVar _ | TUnit | TVar _ | TArrow _ | TLabel _ | TData _ | TApp _ -> [ tr_type tp ]
...
and tr_type_app : type k. k typ -> SExpr.t list -> SExpr.t =
  ...
  | TUVar _ | TUnit | TEffPure | TEffJoin _ | TVar _ | TArrow _ | TForall _ | TLabel _
  | TData _ ->
