...
(** Finalize checking of ADT definition. It extends environment by ADT
  metadata (proof) and constructors. Constructors may shadow some names in
  parameter environment. The [nonrec_scope] parameter is a scope not extended
  with (mutually-)recursive definitions, and is used to check if a type is
  strictly positively recursive. Note that during type inference, the type is
  considered as strictly positively recursive if it does not contain type
  variables on non-strictly positive position or unification variables that do
