...
(* ========================================================================== *)
(** Lookup a type in the environment. Returns [None] if the type is not
  bound. For other exceptional situations raises an error. *)
val try_lookup_tvar :
  pos:Position.t -> 'st Env.t -> S.tvar -> T.typ option

(** Lookup a type in the module. Returns [None] if the type is not bound.
...
(** Lookup a value in the environment. Returns [None] if the value is not
  bound. For other exceptional situations raises an error. *)
val try_lookup_val :
  pos:Position.t -> 'st Env.t -> Name.t -> (T.poly_expr * T.scheme) option

(** Lookup a value in the module. Returns [None] if the value is not bound.
  For other exceptional situations raises an error. *)
val try_lookup_val_in_module :
  pos:Position.t -> 'st Env.t -> closed Module.t -> Name.t ->
    (T.poly_expr * T.scheme) option

(** Lookup the implicit value and extract a variable and its scheme.
...
(** Lookup implicit "label" variable and extract its unif representation.
...
(** Lookup a method and extract a variable and its scheme.
