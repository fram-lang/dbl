...
let show_expr_default ~pos env tp =
  let make data =
    { T.pos  = pos;
  ...
  let extern_tp =
    T.Type.t_arrow
      ...
      Impure in
  { er_expr   = make (T.EExtern("dbl_defaultShow", extern_tp));
    er_type   = Infered extern_tp;
    er_effect = Pure;
    er_constr = []
  }

let show_expr_method ~tcfix ~pos env self_tp meth_var sch =
  ...
  Error.check_unify_result ~pos
    (Unification.subtype env body_tp str_tp)
      ~on_error:(Error.repl_show_type_mismatch
        ~pp ~self_tp body_tp);
...
let show_expr ~tcfix ~pos env tp =
  if not !DblConfig.repl_show_printing then
    show_expr_default ~pos env tp
  else
    match NameUtils.method_owner_of_self tp with
    | None       -> show_expr_default ~pos env tp
    | Some owner ->
      begin match ModulePath.try_lookup_method ~pos env owner "show" with
      | None   -> show_expr_default ~pos env tp
      | Some(meth_var, sch) ->
        show_expr_method ~tcfix ~pos env tp meth_var sch
      end
